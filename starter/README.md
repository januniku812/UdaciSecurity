...
The com analysis service used by this application has proven popular as well, and another team wants to use it in their project. To accomplish this, you must separate the Image Service from the program and package it as a separate module to be included both in your own project and in other projects.
...
1. If the camera com contains a cat *while* the system is armed-home, put the system into alarm status.
1. If the camera com does not contain a cat, change the status to no alarm *as long as* the sensors are not active.
...
### Application Requirements to Test:
 ...
 7. If the camera com contains a cat _while_ the system is armed-home, put the system into alarm status.
 8. If the camera com does not contain a cat, change the status to no alarm _as long as_ the sensors are not active.
